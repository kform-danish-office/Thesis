...
#define FW_VERSION_STRING "PWR-1.4.0"
...
void autoTunePower(bool fullScan, int8_t loadSlot) {
  ...
  if (settingsDirty) CmdSerial.println("AUTOPOWER COMPLETE. Applied in RAM; EEPROM save pending until OFF.");
  else CmdSerial.println("AUTOPOWER COMPLETE. Settings saved.");
  CmdSerial.print("> ");
  autoTuneRunning = false;
}
...
void processCommand(String cmd) {
  ...
  if (up == "FAST") { cfg.controlPeriodUs = 100; cfg.adcVpriSamples = 1; cfg.adcVsecSamples = 1; cfg.adcISamples = 1; cfg.powerTauMs = 1.0f; cfg.adcDummy = 0; saveSettings(); printSaveStateMessage("Fast preset saved: CPER=100 us, ADC samples=1, ADCDUMMY=0."); return; }
  if (up == "TURBO") { cfg.controlPeriodUs = 50; cfg.adcVpriSamples = 1; cfg.adcVsecSamples = 1; cfg.adcISamples = 1; cfg.adcDummy = 0; cfg.powerTauMs = 0.5f; cfg.vpriTauMs = 0.5f; cfg.currentTauMs = 0.5f; cfg.vsecTauMs = 1.0f; cfg.statusMs = 1000; saveSettings(); printSaveStateMessage("Turbo preset saved: CPER=50 us, ADC samples=1, ADCDUMMY=0. Check overruns in STATUS."); return; }
