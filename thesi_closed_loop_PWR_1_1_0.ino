...
//
// Firmware version:
//   PWR-1.1.0 = adds version reporting and secondary-feedback precharge.
//
// Fixed carrier:
...
#define FW_VERSION_STRING "PWR-1.1.0"
...
#define EEPROM_MAGIC       0x50575232UL  // "PWR2"
#define EEPROM_MAGIC_NAME  "PWR2"
...
#define DEFAULT_PRECHARGE_DUTY_X10    5    // 0.5%; min-edge clamp makes this about 2 us at 10 kHz
...
void updateEnableTimingNoInterruptLock() {
  ...
  uint16_t cmd = rawCmd;
  if (rawCmd != 0) cmd = clampControlDutyLimit(cmd);
  ...
  enPeriodUs = period;
  enHighUs = high;
  enDutyEffX10 = (period == 0) ? 0 : (uint16_t)((1000UL * high + period / 2UL) / period);
}
...
void setEnDutyX10(uint16_t dutyX10) {
  ...
  updateEnableTimingNoInterruptLock();
  if (enHighUs != enTimerAppliedHighUs) applyEnableTimerNoInterruptLock(false);
...
void startStartupBurst() {
  ...
  setEnDutyX10(cfg.prechargeDutyX10);
}
...
void printParams() {
  ...
  CmdSerial.println("PRECHEN, PREDUTY, PREMS, PRESETTLE, PREMAX, PREV");
  ...
  CmdSerial.println("Commands: ON, OFF, OPEN, POWER, CAL/AUTOFULL, CALLOAD n, CAL3, LOAD n, ZEROI, CLR, SAVE, DEFAULTS");
}
...
void processCommand(String cmd) {
  ...
  if (up == "FAST") { cfg.controlPeriodUs = 100; cfg.adcVpriSamples = 1; cfg.adcVsecSamples = 1; cfg.adcISamples = 1; cfg.powerTauMs = 1.0f; saveSettings(); CmdSerial.println("Fast preset saved: CPER=100 us, ADC samples=1."); return; }
...
void handleSerial() {
  if (millis() - lastUartCharMs > 5000UL && inputLine.length() == 0) uartInputActive = false;
