...
#define FW_VERSION_STRING "PWR-1.7.1"
...
#define EN_MIN_FULL_GATE_CYCLES    2UL
...
uint32_t minCompleteGateBurstUs() {
  uint32_t gatePeriodUs = (1000000UL + GATE_FREQ_HZ - 1UL) / GATE_FREQ_HZ;
  if (gatePeriodUs < 1UL) gatePeriodUs = 1UL;
  return gatePeriodUs * EN_MIN_FULL_GATE_CYCLES;
}
...
void updateEnableTimingNoInterruptLock() {
  ...
  uint32_t minGateBurst = minCompleteGateBurstUs();
  if (minEdge < minGateBurst) minEdge = minGateBurst;
...
void printStatus() {
  ...
  CmdSerial.print("PA10 period/high/minedge/gatemin/static100: ");
