...
#define FW_VERSION_STRING "PWR-1.8.3"
...
#define EEPROM_MAGIC       0x50575241UL  // "PWRA"
#define EEPROM_MAGIC_NAME  "PWR10"
...
void handlePowerLoop() {
  ...
  bool lightLoad = lightLoadStateActive();
  if (lightLoad && filteredVsec >= cfg.secMinV + LOW_LOAD_VSEC_RELEASE_V) forceLow = true;
  ...
  if (forceLow && bangBangHigh) {
    ...
    bool needEnergy = filteredPower <= lowThreshold;
    if (lightLoad && filteredVsec >= cfg.secMinV) needEnergy = false;
    if (!forceLow && needEnergy && dwellMs >= cfg.bangMinOffMs) {
      bangBangHigh = true;
      bangBangLastSwitchMs = nowMs;
    }
  }
...
void printLiveStatusLine() {
  ...
  if (lightLoadStateActive()) CmdSerial.print(" LLOAD");
...
void printStatus() {
  ...
  CmdSerial.print("Light-load mode/threshold: ");
  CmdSerial.print(lightLoadStateActive() ? "YES" : "NO");
  CmdSerial.print(" / ");
  CmdSerial.print(LOW_LOAD_DETECT_OHMS, 1);
  CmdSerial.print(" ohm or ");
  CmdSerial.print(LOW_LOAD_DETECT_CURRENT_A, 3);
  CmdSerial.println(" A in voltage band");
