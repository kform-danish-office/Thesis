...
#define FW_VERSION_STRING "PWR-1.9.3"
...
#define EEPROM_MAGIC       0x50575244UL  // "PWRD"
#define EEPROM_MAGIC_NAME  "PWR13"
...
#define DEFAULT_NO_LOAD_BAND_V           1.0f // release at VMIN + this voltage
...
void validateSettings() {
  ...
  if (!isfinite(cfg.noLoadBandV) || cfg.noLoadBandV < 0.2f || cfg.noLoadBandV > 20.0f) cfg.noLoadBandV = DEFAULT_NO_LOAD_BAND_V;
...
bool lightLoadStateActive() {
  ...
  if (slotR >= LOW_LOAD_DETECT_OHMS) return true;
...
float noLoadReleaseVoltage() {
  float releaseV = cfg.secMinV + cfg.noLoadBandV;
  if (releaseV > cfg.secMaxV) releaseV = cfg.secMaxV;
  if (releaseV < cfg.secMinV + 0.2f) releaseV = cfg.secMinV + 0.2f;
  return releaseV;
}

bool noLoadGuardActive() {
  if (!cfg.noLoadGuardEnable) return false;
  float fastVsec = (latestVsecRaw > 0.05f) ? latestVsecRaw : filteredVsec;
...
void handlePowerLoop() {
  ...
  bool noLoadGuard = (!underVoltage) && noLoadGuardActive();
  ...
  if (underVoltage) {
    ...
    if (underVoltage && dwellMs >= cfg.bangMinOffMs) {
      bangBangHigh = true;
      bangBangLastSwitchMs = nowMs;
    }
  }
...
void printParams() {
  ...
  CmdSerial.println("VMIN, VMAX, OVP, OVPEN, NLGUARD, NLDUTY, NLBAND, NLI, NLR");
...
void printStatus() {
  ...
  CmdSerial.print("No-load guard en/duty/release/I/R/state: ");
  ...
  CmdSerial.println(noLoadGuardActive() ? "ACTIVE" : "idle");
...
bool parseSetParam(String name, float value) {
  ...
  else if (name == "NLBAND" || name == "NLV") cfg.noLoadBandV = clampFloat(value, 0.2f, 20.0f);
