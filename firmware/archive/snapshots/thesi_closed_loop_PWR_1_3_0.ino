...
//
// Firmware version:
//   PWR-1.3.0 = ISR/serial hardening, bounded UART input, and TURBO loop preset.
//
// Fixed carrier:
...
//
// Feedback:
//   PA0 = primary voltage divider into MCU ADC
//   PA1 = isolated secondary voltage monitor, guard only
//   PA2 = primary / half-bridge current monitor
//
// Objective:
//   regulate primary-side input power = Vpri * Ipri.
//   Secondary voltage is only guarded: desired band 10..28 V, OVP 31 V.
// ============================================================
...
#define FW_VERSION_STRING "PWR-1.3.0"
...
#define EEPROM_MAGIC       0x50575233UL  // "PWR3"
#define EEPROM_MAGIC_NAME  "PWR3"
...
#define DEFAULT_DEADBAND_MW        250L   // 0.250 W
...
void updateEnableTimingNoInterruptLock() {
  ...
  uint32_t minEdge = clampU32(cfg.enMinEdgeUs, EN_MIN_EDGE_HARD_US, EN_MAX_EDGE_HARD_US);
  ...
  if (rawCmd == 0 || cmd == 0) high = 0;
    ...
    if (requestedHighX1000 > 0ULL && requestedHighX1000 < (uint64_t)minEdge * 1000ULL) {
      uint64_t skip = (((uint64_t)minEdge * 1000ULL) + requestedHighX1000 - 1ULL) / requestedHighX1000;
...
uint16_t readAdcAveragedFast(uint32_t pin, uint16_t samples) {
  if (cfg.adcDummy) {
    for (uint16_t i = 0; i < cfg.adcDummy; i++) (void)analogRead(pin);
  }
  uint32_t sum = 0;
  if (samples < 1) samples = 1;
  for (uint16_t i = 0; i < samples; i++) sum += (uint16_t)analogRead(pin);
...
float readPrimaryVoltageRaw() {
  rawPa0Adc = readAdcAveragedFast(SENSE_VPRI_PIN, cfg.adcVpriSamples);
...
float readSecondaryVoltageRaw() {
  rawPa1Adc = readAdcAveragedFast(SENSE_VOUT_PIN, cfg.adcVsecSamples);
...
void savePrechargeTuneResult(uint16_t dutyX10, uint8_t bursts, float vAfter) {
  ...
  CmdSerial.println();
  CmdSerial.println("AUTOPRE RESULT SAVED");
...
void setMode(ControlMode mode, bool save) {
  ...
  powerIntegratorPct = 0.0f;
  controlDutyPercent = dutyX10ToPercent(cfg.biasDutyX10);
  if (systemEnabled) {
    if (!startupPrechargeActive()) {
      if (mode == MODE_OPEN_LOOP) setEnDutyX10(cfg.openLoopDutyX10);
      else setEnDutyX10(cfg.powerupDutyX10);
    }
  }
  updateModeLEDs();
  if (save) saveSettings();
...
void handlePowerLoop() {
  ...
  float dbW = ((float)cfg.deadbandMw) / 1000.0f;
  float ctrlErr = (fabsf(errW) <= dbW) ? 0.0f : errW;

  float kp = ((float)cfg.kp_x1000) / 1000.0f;
  float ki = ((float)cfg.ki_x1000) / 1000.0f;
  powerIntegratorPct += ki * ctrlErr * dt;
  powerIntegratorPct = clampFloat(powerIntegratorPct,
                                  ((float)cfg.integralMinX10) / 10.0f,
                                  ((float)cfg.integralMaxX10) / 10.0f);

  float desired = dutyX10ToPercent(cfg.biasDutyX10) + kp * ctrlErr + powerIntegratorPct;
  desired += lowVoltageBoostPct();
  desired += highVoltageFoldbackPct();
  desired += currentFoldbackPct();

  float minPct = dutyX10ToPercent(cfg.dutyMinX10);
  float maxPct = dutyX10ToPercent(cfg.dutyMaxX10);
  desired = clampFloat(desired, minPct, maxPct);

  float maxStep = (((float)cfg.slewX10PerSec) / 10.0f) * dt;
  if (maxStep < 0.01f) maxStep = 0.01f;
  if (desired > controlDutyPercent + maxStep) desired = controlDutyPercent + maxStep;
  if (desired < controlDutyPercent - maxStep) desired = controlDutyPercent - maxStep;

  controlDutyPercent = desired;
...
MeasureResult measureWindow(uint32_t ms) {
  ...
  m.valid = m.avgVpri > 1.0f && m.avgI >= 0.0f && m.avgP >= 0.0f;
  return m;
}
...
MeasureResult settleAndMeasure(uint32_t settleMs, uint32_t measureMs) {
  if (!serviceDelayMs(settleMs)) {
    ...
    m.valid = filteredVpri > 1.0f && filteredCurrent >= 0.0f && filteredPower >= 0.0f;
...
void printMeasure(const char *tag, uint32_t freqHz, uint16_t dutyX10, const MeasureResult &m, float score) {
  ...
  CmdSerial.print("% P="); CmdSerial.print(m.avgP, 3);
...
bool autotunePowerFull(int8_t loadSlot, uint32_t &bestFreq, uint16_t &bestDuty, MeasureResult &bestMeasure) {
  CmdSerial.println();
  CmdSerial.println("AUTOPOWER FULL: trying EN frequency and duty grid");
  CmdSerial.print("F range "); CmdSerial.print(cfg.enFreqMinHz);
  CmdSerial.print("-"); CmdSerial.print(cfg.enFreqMaxHz);
  CmdSerial.print(" Hz, preferred "); CmdSerial.print(cfg.enFreqPrefHz);
  ...
  bestDuty = cfg.biasDutyX10;
  float bestScore = 999999.0f;
  initMeasure(bestMeasure);

  uint32_t stepF = cfg.autotuneFreqStepHz;
  if (stepF < 500UL) stepF = 500UL;
  bool ok = true;

  for (uint32_t f = cfg.enFreqMinHz; f <= cfg.enFreqMaxHz; ) {
    for (uint16_t d = loDuty; d <= hiDuty; ) {
      ok = considerTunePoint("GRID", f, d, loadSlot, bestFreq, bestDuty, bestScore, bestMeasure);
      if (!ok) return false;
      if (lastTunePointHighVStop) break;
      if (hiDuty - d < cfg.autotuneCoarseStepX10) break;
      d = (uint16_t)(d + cfg.autotuneCoarseStepX10);
    }
    if (cfg.enFreqMaxHz - f < stepF) break;
    f += stepF;
  }

  // Always evaluate the preferred 10 kHz point and the current point.
  (void)considerTunePoint("PREF", clampU32(cfg.enFreqPrefHz, cfg.enFreqMinHz, cfg.enFreqMaxHz), bestDuty, loadSlot, bestFreq, bestDuty, bestScore, bestMeasure);
  (void)considerTunePoint("CURR", cfg.enFreqHz, bestDuty, loadSlot, bestFreq, bestDuty, bestScore, bestMeasure);
  ...
  for (uint16_t d = (uint16_t)start; d <= (uint16_t)stop; ) {
    ok = considerTunePoint("FINE", bestFreq, d, loadSlot, bestFreq, bestDuty, bestScore, bestMeasure);
    if (!ok) return false;
    if (lastTunePointHighVStop) break;
...
void autoTunePower(bool fullScan, int8_t loadSlot) {
  ...
  CmdSerial.println();
  CmdSerial.println("================================================");
  CmdSerial.println("AUTOPOWER START: PA10 EN is power throttle");
  CmdSerial.println("================================================");
  ...
  if (!runStartupPrechargeBlocking()) {
    setEnDutyX10(0);
    autoTuneRunning = false;
    CmdSerial.println("AUTOPOWER FAILED: startup precharge fault.");
    CmdSerial.print("> ");
    return;
  }
  ...
  if (!autotunePowerFull(loadSlot, bestFreq, bestDuty, bestMeasure)) {
    setEnDutyX10(0);
    autoTuneRunning = false;
    CmdSerial.println("AUTOPOWER FAILED: no safe power point found.");
    CmdSerial.print("> ");
    return;
  }
  ...
  if (!plantOk) {
    CmdSerial.println("Power plant ID weak; using conservative fallback.");
    gain = (bestDuty > 10 && bestMeasure.avgP > 0.5f) ? bestMeasure.avgP / dutyX10ToPercent(bestDuty) : 0.20f;
    if (gain < 0.02f) gain = 0.20f;
    tau = 0.20f;
  }
  ...
  cfg.tunedPlantGainWPerPct = gain;
  cfg.tunedPlantTauS = tau;
  if (loadSlot >= 0 && validLoadSlot((uint8_t)loadSlot)) {
    storeLoadProfile((uint8_t)loadSlot, bestFreq, bestDuty, gain, tau, bestMeasure);
  }
  ...
  CmdSerial.println();
  CmdSerial.println("AUTOPOWER RESULT");
  CmdSerial.print("FEN="); CmdSerial.print(bestFreq); CmdSerial.println(" Hz");
  CmdSerial.print("BIAS="); CmdSerial.print(dutyX10ToPercent(bestDuty), 2); CmdSerial.println(" %");
  CmdSerial.print("Measured P="); CmdSerial.print(bestMeasure.avgP, 3); CmdSerial.print(" W, target=");
  ...
  CmdSerial.print(" %/W, Ki="); CmdSerial.print(((float)cfg.ki_x1000) / 1000.0f, 4);
  CmdSerial.println(" %/(W*s)");
  CmdSerial.println("AUTOPOWER COMPLETE. Settings saved.");
  CmdSerial.print("> ");
  autoTuneRunning = false;
}
...
const char *modeName() {
  return controlMode == MODE_OPEN_LOOP ? "OPEN" : "POWER";
}

void printLiveStatusLine() {
  CmdSerial.print("\rP="); CmdSerial.print(filteredPower, 2); CmdSerial.print("W");
...
void printParams() {
  ...
  CmdSerial.println("FEN, FMIN, FMAX, FPREF, FPEN, MINEDGE, STATIC, DMIN, DMAX, OD, BIAS, PWRDUTY");
  CmdSerial.println("CPER, STATUSMS, ADCVP, ADCVS, ADCI, ADCDUMMY, PTAU, VPTAU, VSTAU, ITAU");
  ...
  CmdSerial.println("AUTOMAX, AUTOSTEP, AUTOFINE, AUTOSETTLE, AUTOMEAS, AUTOTEST, AUTOFSTEP");
  CmdSerial.println("LOAD1, LOAD2, LOAD3");
  CmdSerial.println("Commands: ON, OFF, OPEN, POWER, AUTOPRE, CAL/AUTOFULL, CALLOAD n, CAL3, LOAD n, ZEROI, FAST, TURBO, CLR, SAVE, DEFAULTS");
}
...
void printStatus() {
  ...
  CmdSerial.println();
  CmdSerial.println("========== POWER CONTROL STATUS ==========");
  ...
  CmdSerial.print("Measured P/Vpri/I/Vsec: ");
  ...
  CmdSerial.print("PA10 period/high/minedge/static100: ");
  CmdSerial.print(enPeriodUs); CmdSerial.print("/"); CmdSerial.print(enHighUs); CmdSerial.print("/");
  CmdSerial.print(cfg.enMinEdgeUs); CmdSerial.print(" us / "); CmdSerial.println(cfg.allowStatic100 ? "YES" : "NO");
  ...
  CmdSerial.print("Kp/Ki/deadband/slew: ");
  CmdSerial.print(((float)cfg.kp_x1000) / 1000.0f, 4); CmdSerial.print(" / ");
  CmdSerial.print(((float)cfg.ki_x1000) / 1000.0f, 4); CmdSerial.print(" / ");
  CmdSerial.print(((float)cfg.deadbandMw) / 1000.0f, 3); CmdSerial.print(" W / ");
  CmdSerial.print(((float)cfg.slewX10PerSec) / 10.0f); CmdSerial.println(" %/s");
...
bool parseSetParam(String name, float value) {
  ...
  if (name == "PTARGET" || name == "PWR" || name == "P") cfg.targetPowerW = clampFloat(value, POWER_TARGET_MIN_W, POWER_TARGET_MAX_W);
...
void printLoadCommandHelp() {
  CmdSerial.println("Use LOAD 1..3, CALLOAD 1..3, AUTOFULL 1..3, CAL3, or SET LOAD1/2/3 ohms.");
}
...
void processCommand(String cmd) {
  ...
  if (up == "POWER" || up == "MC" || up == "CLOSED") { setMode(MODE_POWER_LOOP, true); CmdSerial.println("POWER LOOP"); return; }
  if (up == "CAL" || up == "AUTOTUNE" || up == "AUTOFULL") { autoTunePower(true, (int8_t)cfg.activeCalLoadSlot); return; }
  if (up == "CAL3") { autoTuneThreeLoads(); return; }
  if (up == "SAVE") { saveSettings(); CmdSerial.println("Saved."); return; }
  if (up == "DEFAULTS") { setDefaults(); validateSettings(); saveSettings(); setMode(MODE_OPEN_LOOP, false); setSystemEnabled(false, false); CmdSerial.println("Defaults restored and saved."); return; }
  ...
  if (up == "FAST") { cfg.controlPeriodUs = 100; cfg.adcVpriSamples = 1; cfg.adcVsecSamples = 1; cfg.adcISamples = 1; cfg.powerTauMs = 1.0f; cfg.adcDummy = 0; saveSettings(); CmdSerial.println("Fast preset saved: CPER=100 us, ADC samples=1, ADCDUMMY=0."); return; }
  if (up == "TURBO") { cfg.controlPeriodUs = 50; cfg.adcVpriSamples = 1; cfg.adcVsecSamples = 1; cfg.adcISamples = 1; cfg.adcDummy = 0; cfg.powerTauMs = 0.5f; cfg.vpriTauMs = 0.5f; cfg.currentTauMs = 0.5f; cfg.vsecTauMs = 1.0f; cfg.statusMs = 1000; saveSettings(); CmdSerial.println("Turbo preset saved: CPER=50 us, ADC samples=1, ADCDUMMY=0. Check overruns in STATUS."); return; }
  if (up == "SAFE") { cfg.dutyMaxX10 = 500; cfg.autotuneMaxDutyX10 = 400; cfg.allowStatic100 = 0; cfg.maxDutyFaultEnable = 1; cfg.currentTripEnable = 1; saveSettings(); CmdSerial.println("Safe preset saved."); return; }
  ...
  // Short aliases
  if (up.startsWith("P ")) { if (parseSetParam("PTARGET", cmd.substring(2).toFloat())) CmdSerial.println("Power target saved."); return; }
  if (up.startsWith("D ")) { if (parseSetParam("OD", cmd.substring(2).toFloat())) CmdSerial.println("Open-loop duty saved."); return; }
  if (up.startsWith("F ")) { if (parseSetParam("FEN", cmd.substring(2).toFloat())) CmdSerial.println("EN frequency saved."); return; }
...
void setup() {
  ...
  CmdSerial.println("PA8/PA9 fixed 1 MHz. PA10 EN is power throttle, default 10 kHz.");
  CmdSerial.println("PA0 primary voltage, PA1 secondary guard, PA2 current. Type ? for status, VER for version.");
  printDividerAdvice();
  printStatus();
  CmdSerial.print("> ");
}
