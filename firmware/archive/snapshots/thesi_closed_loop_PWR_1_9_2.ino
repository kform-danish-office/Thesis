...
#define FW_VERSION_STRING "PWR-1.9.2"
...
#define EEPROM_MAGIC       0x50575243UL  // "PWRC"
#define EEPROM_MAGIC_NAME  "PWR12"
...
#define DEFAULT_NO_LOAD_DUTY_X10       120   // 12.0% EN cap while no-load guard is active
#define DEFAULT_NO_LOAD_BAND_V           2.0f // release at VMIN + this voltage
#define DEFAULT_NO_LOAD_CURRENT_A        0.12f
#define DEFAULT_NO_LOAD_OHMS           120.0f
...
bool noLoadGuardActive() {
  ...
  if (filteredVsec < cfg.prechargeReadyV) return false;
  if (filteredVsec <= cfg.secMinV || latestVsecRaw <= cfg.secMinV) return false;
...
void handlePowerLoop() {
  ...
  bool underVoltage = (filteredVsec <= cfg.secMinV) || (latestVsecRaw <= cfg.secMinV);
  ...
  if (underVoltage) {
    ...
    if (filteredVsec >= releaseV && dwellMs >= cfg.bangMinOnMs) {
      bangBangHigh = false;
      bangBangLastSwitchMs = nowMs;
    }
  } else {
    if (filteredVsec <= cfg.secMinV && dwellMs >= cfg.bangMinOffMs) {
      bangBangHigh = true;
      bangBangLastSwitchMs = nowMs;
    }
  }
