...
//   PA1 = isolated secondary voltage monitor / optional power feedback voltage
//   PA2 = primary / half-bridge current monitor
//
// Objective:
//   regulate selected feedback power = Vpri * Ipri or Vsec * current feedback.
...
#define FW_VERSION_STRING "PWR-1.5.1"
...
#define EEPROM_MAGIC       0x50575234UL  // "PWR4"
#define EEPROM_MAGIC_NAME  "PWR4"
...
#define DEFAULT_POWER_FEEDBACK_MODE PFB_PRIMARY_VI
...
float scorePowerPoint(uint32_t freqHz, uint16_t dutyX10, const MeasureResult &m, int8_t loadSlot) {
  ...
  if (cfg.powerFeedbackMode == PFB_PRIMARY_VI && loadSlot >= 0 && validLoadSlot((uint8_t)loadSlot)) {
...
void printParams() {
  ...
  CmdSerial.println("Commands: ON, OFF, OPEN, POWER, PFBPRI, PFBSEC, AUTOPRE, CAL/AUTOFULL, CALLOAD n, CAL3, LOAD n, ZEROI, FAST, TURBO, CLR, SAVE, DEFAULTS");
}
