...
#define FW_VERSION_STRING "PWR-1.9.4"
...
#define EEPROM_MAGIC       0x50575245UL  // "PWRE"
#define EEPROM_MAGIC_NAME  "PWR14"
...
#define DEFAULT_VSEC_TAU_MS           0.8f
...
#define DEFAULT_NO_LOAD_BAND_V          10.0f // no-load low floor is VSET minus this voltage
...
void handlePowerLoop() {
  ...
  noLoadForceFull = cfg.noLoadGuardEnable &&
                    cfg.noLoadFullCycles > 0 &&
                    noLoadBelowMinCycles >= cfg.noLoadFullCycles;
  bool noLoadGuard = (!underVoltage) && (!noLoadForceFull) && noLoadGuardActive();
  float releaseV = (noLoadGuard || noLoadForceFull) ? noLoadReleaseVoltage() : cfg.secMaxV;
  float turnOnV = cfg.secMinV;
  if (noLoadGuard || noLoadForceFull) turnOnV = noLoadLowV;
  bool belowTurnOn = (filteredVsec <= turnOnV) || (fastVsec <= turnOnV);
  bool overRelease = (filteredVsec >= releaseV) || (latestVsecRaw >= releaseV);

  if (underVoltage || (noLoadForceFull && belowTurnOn)) {
    if (!bangBangHigh) {
      bangBangHigh = true;
    ...
    if (belowTurnOn && dwellMs >= cfg.bangMinOffMs) {
      bangBangHigh = true;
      bangBangLastSwitchMs = nowMs;
    }
  }
