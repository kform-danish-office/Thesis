...
#define FW_VERSION_STRING "PWR-1.9.6"
...
#define EEPROM_MAGIC       0x50575247UL  // "PWRG"
#define EEPROM_MAGIC_NAME  "PWR16"
...
void handlePowerLoop() {
  ...
  if (bangBangHigh) {
    ...
    if (underVoltage || noLoadForceFull || (belowTurnOn && dwellMs >= cfg.bangMinOffMs)) {
      bangBangHigh = true;
      bangBangLastSwitchMs = nowMs;
    }
  }
...
float scorePowerPoint(uint32_t freqHz, uint16_t dutyX10, const MeasureResult &m, int8_t loadSlot) {
  ...
  float targetV = cfg.secMaxV - 0.5f;
